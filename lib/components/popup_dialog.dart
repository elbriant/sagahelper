...
abstract final class PopupDialog {
  ...
  static final navContext = NavigationService.navigatorKey.currentContext!;
...
class _EntityViewWidget extends ConsumerWidget {
  final Entity entity;

  const _EntityViewWidget({
    required this.entity,
  });

  String getStat(String stat, int elite, int level) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final tagsAsArknigts = ref.watch(styleProvider).tagsAsArknights;
          ...
          children: [
            ...
            Table(
              ...
              children: [
                TableRow(
                  children: [
                    StatTileList(stat: 'HP', value: getStat('maxHp', elite, level)),
                    StatTileList(
                      stat: 'Redeploy',
                      value: '${getStat('respawnTime', elite, level)} sec',
                    ),
                  ],
                ),
                TableRow(
                  children: [
                    StatTileList(stat: 'ATK', value: getStat('atk', elite, level)),
                    StatTileList(stat: 'Block', value: getStat('blockCnt', elite, level)),
                  ],
                ),
                TableRow(
                  children: [
                    StatTileList(stat: 'DEF', value: getStat('def', elite, level)),
                    StatTileList(stat: 'DP Cost', value: getStat('cost', elite, level)),
                  ],
                ),
                TableRow(
                  children: [
                    StatTileList(
                      stat: 'RES',
                      value: '${getStat('magicResistance', elite, level)}%',
                    ),
                    StatTileList(
                      stat: 'ASPD',
                      value: '${getStat('baseAttackTime', elite, level)} sec',
                    ),
                  ],
                ),
              ],
            ...
            // talents if any
            Visibility(
              visible: entity.talents.isNotNull,
              child: Builder(
                builder: (context) {
                  ...
                  List<Widget?> talentWidgets = List.generate(entity.talents!.length, (index) {
                    if (entity.talents![index]["candidates"] == null) {
                      return null;
                    }

                    Map? candidate = (entity.talents![index]["candidates"] as List).lastWhere(
                      (candidate) {
                        int thisTalentCandidateElite = int.parse(
                          (candidate["unlockCondition"]["phase"] as String)
                              .replaceFirst('PHASE_', ''),
                        ...
                        child: Column(
                          ...
                          children: [
                            AnimatedContainer(
                              ...
                              child: StyledText(
                                text:
                                    (entity.talents![index]["candidates"] as List).first["name"] ??
                                        '<i-sub> no name </i-sub>',
                                style: TextStyle(
                                  color: unlocked
            ...
            // skills if any
            Visibility(
              visible: entity.skills.isNotNull,
              child: Consumer(
                builder: (context, ref, child) {
                  ...
                  Map skillDetail =
                      ref.read(cacheProvider).cachedSkillTable![selectedSkill["skillId"]];
                  ...
                  Map skillDetailLv = ref
                      .read(cacheProvider)
                      .cachedSkillTable![selectedSkill["skillId"]]["levels"][currentSkillLevel];
                                          ...
                                          child: StoredFadeInImage(
                                            imageUrl:
                                                '$kSkillRepo/skill_icon_${skillDetail["iconId"] ?? skillDetail["skillId"]}.png'
                                                    .githubEncode(),
                                            filename:
                                                '${skillDetail["iconId"] ?? skillDetail["skillId"]}.png',
