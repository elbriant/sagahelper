...
class StyleProvider extends ChangeNotifier {
  ...
  Map<String, TextStyle> statsStyles({BuildContext? context}) => {
    'HP' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sHp),
    'ATK' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sAtk),
    'DPCost' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sCost),
    'Redeploy' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sRedeploy),
    'DEF' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sDef),
    'RES' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sRes),
    'Block' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sBlock),
    'ASPD' : TextStyle(color: StaticColors.fromBrightness(Theme.of(context ?? navContext).brightness).sAspd),
  };
      ...
      parse: (baseStyle, attributes) {
        if (attributes.containsKey('name') && (attributes['name']!.substring(0, 1) == '#') && attributes['name']!.length >= 6) {
          ...
          if (hexColor.toLowerCase() != 'ffffff' && hexColor.toLowerCase() != 'ffffffff') {
            ...
            return baseStyle!.copyWith(color: Theme.of(context ?? navContext).colorScheme.secondary);
          }
        } else {
          return baseStyle;
        }
